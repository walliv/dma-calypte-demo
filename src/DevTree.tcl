proc dts_build_netcope {} {
    ...
    # DMA module
    global DMA_TYPE DMA_RX_CHANNELS DMA_TX_CHANNELS PCIE_ENDPOINTS DMA_RX_PKT_SIZE_MAX DMA_TX_PKT_SIZE_MAX DMA_RX_PKT_SIZE_MIN DMA_TX_PKT_SIZE_MIN
    if {$DMA_TYPE != 0} {
        append ret [dts_dmamod_open $ADDR_DMA_MOD $DMA_TYPE [expr $DMA_RX_CHANNELS / $PCIE_ENDPOINTS] [expr $DMA_TX_CHANNELS / $PCIE_ENDPOINTS] $mi_idx $DMA_RX_PKT_SIZE_MAX $DMA_TX_PKT_SIZE_MAX $DMA_RX_PKT_SIZE_MIN $DMA_TX_PKT_SIZE_MIN]
    }

    # System Monitor module (SYSMON)
    ...
    global APP_CORE_ARCH HBM_CHANNELS
    if {$APP_CORE_ARCH != "EMPTY" && [llength [info procs dts_application]] > 0 } {
        array set GENERICS "
    ...
    for {set i 0} {$i < $PCIE_ENDPOINTS} {incr i} {
    ...
    for {set i $mi_idx} {$i < $PCIE_ENDPOINTS} {incr i} {
        ...
        if {$DMA_TYPE != 0} {
            append ret [dts_dmamod_open $ADDR_DMA_MOD $DMA_TYPE [expr $DMA_RX_CHANNELS / $PCIE_ENDPOINTS] [expr $DMA_TX_CHANNELS / $PCIE_ENDPOINTS] $i $DMA_RX_PKT_SIZE_MAX $DMA_TX_PKT_SIZE_MAX $DMA_RX_PKT_SIZE_MIN $DMA_TX_PKT_SIZE_MIN]
        }
        append ret "};"
    }
